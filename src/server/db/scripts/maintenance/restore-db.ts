...
// R2 Configuration using the BACKUP_ prefix
...
async function restoreDatabase(backupKey?: string) {
  ...
  if (!selectedKey) {
    ...
    console.log("\n--- Backups Disponíveis (Lente Clínica) ---");
    ...
    console.log(
      `${head.id.padEnd(4)} | ${head.file.padEnd(60)} | ${head.size.padEnd(10)} | ${head.date.padEnd(10)} | ${head.time}`,
    );
    console.log(
      `${"-".repeat(4)}-+-${"-".repeat(60)}-+-${"-".repeat(10)}-+-${"-".repeat(10)}-+-${"-".repeat(8)}`,
    );

    const displayBackups = backups.slice(-15);
    displayBackups.forEach((b, i) => {
      const id = (i + 1).toString().padEnd(4);
      const file = (b.Key?.replace(`${folderName}/`, "") || "").padEnd(60);
  ...
  try {
    ...
    console.log(
      "⏳ Limpando banco de dados atual (Resetando schema public)...",
    );
    // Nota: O DATABASE_URL do Neon já costuma vir com os parâmetros necessários.
