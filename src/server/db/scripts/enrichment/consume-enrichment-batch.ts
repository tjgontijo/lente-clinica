...
import { MEDICATION_ENRICHMENT_PROMPT_VERSION } from "@/features/medications/prompts/medication-enrichment.prompt";
...
async function main() {
  ...
  for (const batchId of batchIds) {
    ...
    if (batch.status !== "completed") {
      console.log(
        `⏳ Batch ${batchId} is still ${batch.status}. Try again later.`,
      );
      continue;
    }
    ...
    console.log(
      `✨ Batch completed! Downloading results from ${batch.output_file_id}...`,
    );
    ...
    for (const line of lines) {
      ...
      if (response.status_code !== 200) {
        console.error(
          `❌ Error in result for ${medId}: Status ${response.status_code}`,
        );
        await db
          .update(medication)
          .set({
      ...
      try {
        ...
        await db
          ...
          .set({
            ...validated,
            // Limpar campos da v4 para evitar dados duplicados/obsoletos
            clinicalContexts: null,
            patientReports: null,
            careObservations: null,
            clinicalConfounders: null,
            usefulQuestions: null,
            ...
            enrichmentPromptVersion: MEDICATION_ENRICHMENT_PROMPT_VERSION,
          })
          .where(eq(medication.id, medId));
