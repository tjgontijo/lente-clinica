...
async function main() {
  ...
  if (!batchId) {
    console.error(
      "❌ Por favor, forneça o ID do batch: npx tsx src/server/db/scripts/relevance/consume-relevance-batch.ts <batch_id>",
    );
    process.exit(1);
  }
  ...
  if (batch.status !== "completed") {
    console.error(
      `❌ Batch ainda não concluído. Status atual: ${batch.status}`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `💡 Nota: Medicamentos com score > 7 foram marcados para enriquecimento automático.`,
  );
}
