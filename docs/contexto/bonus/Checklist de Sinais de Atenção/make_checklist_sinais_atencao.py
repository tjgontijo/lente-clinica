...
def _reg(alias, fname):
    pdfmetrics.registerFont(TTFont(alias, os.path.join(_FD, fname)))

_reg('CG',    'CormorantGaramond-Regular.ttf')
_reg('CG-B',  'CormorantGaramond-SemiBold.ttf')
_reg('CG-I',  'CormorantGaramond-Italic.ttf')
_reg('CG-BI', 'CormorantGaramond-SemiBoldItalic.ttf')
_reg('DMS-L', 'DMSans-Light.ttf')
_reg('DMS',   'DMSans-Regular.ttf')
_reg('DMS-M', 'DMSans-Medium.ttf')
_reg('DMS-B', 'DMSans-SemiBold.ttf')
_reg('DMS-I', 'DMSans-Italic.ttf')
...
# ── Dimensões ─────────────────────────────────────────────────
W, H  = A4
SC    = W / 794   # px → pt  (artboard HTML 794px → A4)
ML = MR = 1.9 * cm
MT    = 2.4 * cm
MB    = 1.5 * cm
CW    = W - ML - MR

# ── Paleta — Design System tokens ─────────────────────────────
S50   = HexColor('#f0f5f2')   # --bg-alt
S100  = HexColor('#f0f5f2')   # --bg-alt
S200  = HexColor('#c8d8d2')   # --border
S300  = HexColor('#c8d8d2')   # --border
S500  = HexColor('#7a9085')   # --text-light
S600  = HexColor('#7a9085')   # --text-light
S700  = HexColor('#3d5248')   # --text-med
S800  = HexColor('#2b4f43')   # --cover-alt
S900  = HexColor('#1e3a32')   # --cover
T400  = HexColor('#e8c4b8')   # --accent-light
T500  = HexColor('#c9725a')   # --accent
T600  = HexColor('#c9725a')   # --accent (barras)
INK   = HexColor('#1a2821')   # --text
INKL  = HexColor('#3d5248')   # --text-med
WHT   = HexColor('#ffffff')
BG    = HexColor('#faf9f7')   # --bg (fundo página)
HLGHT = HexColor('#e8f0ec')   # --highlight (callouts)
...
PHRB  = HexColor('#f0f5f2')   # --bg-alt (phrase cards)

# ── Estilos ───────────────────────────────────────────────────
def ps(n, font='DMS-L', sz=9.5, lead=None, c=None,
       b4=0, a4=0, al=TA_LEFT, li=0, ri=0):
    return ParagraphStyle(n, fontName=font, fontSize=sz,
        leading=lead or round(sz * 1.6), textColor=c or INK,
        spaceBefore=b4, spaceAfter=a4, alignment=al,
        leftIndent=li, rightIndent=ri)

def B(n,  **kw): return ps(n, font='DMS-B',  **kw)
def It(n, **kw): return ps(n, font='DMS-I',  **kw)

ST = {
    # Capa — Cormorant Garamond SemiBold para display
    'cov_title':  ps('cov_title',  font='CG-B',  sz=32, lead=37, c=WHT,  a4=8),
    'cov_sub':    ps('cov_sub',    sz=12, lead=19, c=S200, a4=6),
    'cov_author': ps('cov_author', font='CG',    sz=13, lead=17, c=T400, a4=4),
    'cov_tag':    ps('cov_tag',    sz=8,  lead=12, c=S500),
    # Expediente
    'exp_h1':     B ('exp_h1',     sz=15, lead=22, c=S700, a4=8),
    'exp_h2':     B ('exp_h2',     sz=11, lead=16, c=INK, b4=6, a4=3),
    'exp_body':   ps('exp_body',   sz=10, lead=16, c=INK, a4=3),
    'exp_small':  ps('exp_small',  sz=9,  lead=14, c=INKL, a4=2),
    # Abertura editorial — CG-B para títulos grandes
    'open_h1':    ps('open_h1',    font='CG-B',  sz=24, lead=29, c=INK, a4=8),
    'open_body':  ps('open_body',  sz=10.5, lead=17, c=INK, a4=6),
    'sum_item':   ps('sum_item',   sz=10.5, lead=17, c=INK, a4=2),
    'toc_head':   B ('toc_head',   sz=8,  lead=12, c=INKL),
    'toc_sec':    B ('toc_sec',    sz=10, lead=15, c=INK),
    'toc_sub':    ps('toc_sub',    sz=9.5, lead=14, c=INK),
    'toc_page':   B ('toc_page',   sz=10, lead=15, c=INK, al=TA_RIGHT),
    # Páginas de conteúdo — CG-B para títulos de seção
    'pg_h1':      ps('pg_h1',      font='CG-B',  sz=22, lead=27, c=INK, a4=8),
    'pg_note':    It('pg_note',    sz=9.5, lead=15, c=INKL, li=12, a4=8),
    'intro':      ps('intro',      sz=10,  lead=16, c=INK, a4=6),
    'intro_dest': B ('intro_dest', sz=10,  lead=16, c=S700, a4=6),
    # Cabeçalho de bloco — DMM para tag, CG-B para título
    'blk_tag':    ps('blk_tag',    font='DMM',   sz=8,  lead=11, c=T500),
    'blk_title':  ps('blk_title',  font='CG-B',  sz=28, lead=33, c=INK),
    # Card de sinal
    'sig_h':      B ('sig_h',      sz=10.5, lead=15, c=INK, a4=4),
    'sig_lbl':    B ('sig_lbl',    sz=8,    lead=11, c=INKL, b4=5, a4=2),
    'sig_body':   ps('sig_body',   sz=9.5,  lead=15, c=INK),
    'sig_q':      It('sig_q',      sz=9.5,  lead=15, c=INK, li=8),
    'sig_act':    B ('sig_act',    sz=9,    lead=13, c=INK, b4=5),
    # Card de frase — CG-I para texto da citação (DS: Cormorant italic)
    'phr_lbl':    ps('phr_lbl',    font='DMS',   sz=8,  lead=12, c=WHT),
    'phr_ctx':    ps('phr_ctx',    font='DMM',   sz=7,  lead=10, c=T400),
    'phr_txt':    ps('phr_txt',    font='CG-I',  sz=13, lead=20, c=INK),
    # Comunicação — CG-B para título, CG-I para exemplo
    'comm_h':     ps('comm_h',     font='CG-B',  sz=18, lead=22, c=INK, b4=4, a4=6),
    'comm_body':  ps('comm_body',  sz=9.5,  lead=15, c=INK, a4=4),
    'comm_step':  ps('comm_step',  sz=9.5,  lead=15, c=INK, li=20, a4=3),
    'comm_ex':    ps('comm_ex',    font='CG-I',  sz=12, lead=18, c=S700, li=16, ri=16, b4=6, a4=6),
    # Versão resumida — CG-B para título
    'sum_h':      ps('sum_h',      font='CG-B',  sz=22, lead=27, c=INK, a4=10),
    'sum_th':     B ('sum_th',     sz=8.5, lead=12, c=WHT),
    'sum_td':     ps('sum_td',     sz=8.5, lead=13, c=INK),
    'sum_note':   It('sum_note',   sz=8,   lead=12, c=INKL, b4=8, al=TA_CENTER),
    # CTA final
    'cta_h':      ps('cta_h',      font='CG-B',  sz=16, lead=20, c=S700, al=TA_CENTER, a4=8),
    'cta_body':   ps('cta_body',   sz=10.5, lead=17, c=INK, al=TA_CENTER, a4=6),
...
def draw_cover(c, doc):
    ...
    # Barra accent topo (6px)
    c.setFillColor(T500)
    c.rect(0, H - 6*SC, W, 6*SC, fill=1, stroke=0)

    # Círculos decorativos — S800 (#2b4f43) com opacidade variável
    _cr, _cg, _cb_ = 43/255, 79/255, 67/255
    # Círculo 1: right:-60, top:80, r:210px  → cx=644, cy_top=290
    c.setFillColor(Color(_cr, _cg, _cb_, alpha=0.6))
    c.circle(644*SC, H - 290*SC, 210*SC, fill=1, stroke=0)
    # Círculo 2: right:60, top:220, r:100px  → cx=634, cy_top=320
    c.setFillColor(Color(_cr, _cg, _cb_, alpha=0.4))
    c.circle(634*SC, H - 320*SC, 100*SC, fill=1, stroke=0)
    # Círculo 3: left:-80, bottom:100, r:140px → cx=60, cy_bot=240
    c.setFillColor(Color(_cr, _cg, _cb_, alpha=0.3))
    c.circle(60*SC, 240*SC, 140*SC, fill=1, stroke=0)

    # Barra rodapé (56px, rgba 0,0,0 / 0.25)
    c.setFillColor(Color(0, 0, 0, alpha=0.25))
    c.rect(0, 0, W, 56*SC, fill=1, stroke=0)
    c.setFont('DMS-L', 7)
    c.setFillColor(Color(1, 1, 1, alpha=0.5))
    c.drawCentredString(W/2, 56*SC/2 - 3, 'Uso exclusivo para fins educativos')
    c.setFont('DMS-B', 7)
    c.setFillColor(Color(1, 1, 1, alpha=0.7))
    c.drawRightString(W - 64*SC, 56*SC/2 - 3, 'Dra. Tatiana Gontijo')

    # Bloco de conteúdo (left:64px, bottom:120px — cresce para cima)
    cx = 64 * SC
    y  = 120 * SC

    # Subtítulo do autor (12px DMS-L, branco 60%)
    c.setFont('DMS-L', 9)
    c.setFillColor(Color(1, 1, 1, alpha=0.6))
    c.drawString(cx, y, 'Especialista em Sa\xfade Mental')
    y += 13

    # Nome do autor (22px CG, branco)
    c.setFont('CG', 16.5)
    c.setFillColor(WHT)
    c.drawString(cx, y, 'Dra. Tatiana Gontijo')
    y += 26

    # Divisor (48px × 1.5pt, accent)
    c.setFillColor(T500)
    c.rect(cx, y, 48*SC, 1.5, fill=1, stroke=0)
    y += 14

    # Subtítulo (16px DMS-L, branco 60%)
    c.setFont('DMS-L', 12)
    c.setFillColor(Color(1, 1, 1, alpha=0.6))
    c.drawString(cx, y, 'Ferramenta de apoio \xe0 decis\xe3o cl\xednica')
    y += 22

    # Linha 2 do título: "Sinais de Atenção" (58px CG-BI, accent-light T400)
    c.setFont('CG-BI', 43.5)
    c.setFillColor(T400)
    c.drawString(cx, y, 'Sinais de Aten\xe7\xe3o')
    y += 52

    # Linha 1 do título: "Checklist de" (58px CG-B, branco)
    c.setFont('CG-B', 43.5)
    c.setFillColor(WHT)
    c.drawString(cx, y, 'Checklist de')
    y += 56

    # Badge pill "MATERIAL CLÍNICO EDUCATIVO"
    bw, bh = 150, 19
    c.setFillColor(Color(1, 1, 1, alpha=0.08))
    c.setStrokeColor(T500)
    c.setLineWidth(0.8)
    c.roundRect(cx, y, bw, bh, radius=bh/2, fill=1, stroke=1)
    c.setFont('DMS-M', 7.5)
    c.setFillColor(T400)
    c.drawCentredString(cx + bw/2, y + bh/2 - 3, 'MATERIAL CL\xcdNICO EDUCATIVO')
...
def draw_page(c, doc):
    ...
    c.setFillColor(T500)
    c.circle(ML + 2.2 * mm, MB - 17, 1.8, fill=1, stroke=0)
    c.setFont('DMM', 6.5)
    c.setFillColor(S500)
    c.drawString(ML + 5.5 * mm, MB - 19, 'CHECKLIST DE SINAIS DE ATENCAO')
    c.drawCentredString(W / 2, MB - 19, 'Dra. Tatiana Gontijo')
    c.setFont('DMS-B', 7)
    c.setFillColor(S700)
    c.drawRightString(ML + CW, MB - 19, str(doc.page))
    c.restoreState()
...
def block_header(num, title):
    # DS section-tag: barra accent (24px × 2px) + label uppercase DM Mono
    rule = Table([['']], colWidths=[18 * mm], rowHeights=[1.2 * mm])
    rule.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,-1), T500)]))
    tag = Table([[
        Paragraph(f'SEÇÃO {num:02d}', ST['blk_tag'])
    ]], colWidths=[CW])
    ...
    h = Paragraph(title, ST['blk_title'])
    return [Spacer(1, 4*mm), rule, Spacer(1, 2*mm), tag, Spacer(1, 1.5*mm), h, Spacer(1, 5*mm)]
...
def level_cell(text, color):
    return Paragraph(f'<b>{text}</b>',
        ParagraphStyle('lc', fontName='DMS-B', fontSize=8,
...
def summary_table():
    ...
    row_styles = [
        ...
        ('ROWBACKGROUNDS', (0,1), (-1,-1), [BG, S100]),
    ]
...
def phrase_card(label, text):
    # DS: header escuro (section-bg) + body claro (bg-alt) com CG italic
    card = Table([
        [Paragraph(label, ST['phr_lbl'])],                # row 0: dark header
        [Paragraph('“' + text + '”', ST['phr_txt'])],  # row 1: light body
    ], colWidths=[CW])
    card.setStyle(TableStyle([
        ('BACKGROUND',    (0,0), (0,0), S900),
        ('BACKGROUND',    (0,1), (0,1), PHRB),
        ('BOX',           (0,0), (-1,-1), 0.5, S200),
        ('LINEBELOW',     (0,0), (0,0), 0.5, S800),
...
def compact_cta(url):
    txt = [
        Paragraph('Aprofunde a comunicação com o médico assistente com o Kit de Comunicação Clínica.',
                  ps('cta_inline_b', sz=8.8, lead=13, c=INK, a4=3)),
        Paragraph(f'<link href="{url}" color="#3d5248"><u><b>Clique aqui e conheça</b></u></link>',
                  ps('cta_link', font='DMS-M', sz=9, lead=13, c=S700, a4=0)),
    ]
...
def build_story():
    ...
    # ── CAPA — conteúdo renderizado inteiramente via draw_cover ──
    ...
    s.append(Paragraph(
        ...
        ps('intro_assinatura', font='CG-I', sz=11, lead=15, c=S700, al=TA_RIGHT, a4=0)))
    s.append(PageBreak())
